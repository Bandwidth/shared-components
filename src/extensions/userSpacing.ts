...
const getSize = (value: string | number, props) => {
...
export const calcTopOffset = (lineHeight: number) =>
  `${(lineHeight / -5.25) * (lineHeight / 1.5)}em`;
export const calcBottomOffset = (lineHeight: number) =>
  `${(lineHeight / -3.5) * (lineHeight / 1.5)}em`;

type UserTextSpacingBase = (props: any) => string;
type UserTextSpacingWithLineHeight = (
  lineHeight: number,
) => UserTextSpacingBase;
export interface UserTextSpacing extends UserTextSpacingBase {
  text?: UserTextSpacing;
  withLineHeight?: UserTextSpacingWithLineHeight;
}
...
const userTextSpacingWithLineHeight = (
  lineHeight: number,
): UserTextSpacing => (props: { spacing: Spacing }) => {
  const { spacing } = props;
  ...
  if (isPlainObject(spacing)) {
    ...
    const size = getSize(spacing as string, props);
