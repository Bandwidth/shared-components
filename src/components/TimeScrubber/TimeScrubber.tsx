...
const roundMinutes = (minutes: number) => Math.round(minutes / 15.0) * 15;
const toHours = (momentTime: Moment) =>
  momentTime.hours() + momentTime.minutes() / 60.0;
...
interface TimeScrubberState {
  offset: number;
  dragging: boolean;
  grabPoint: any;
  potentialTime: Moment | null;
...
> {
  ...
  timelineElement: HTMLDivElement | null = null;
  offset = 0;
   ...
   */
  syncTimelinePosition = (timeValue?: Moment) => {
  ...
  handleDrag = ev => {
    ...
    this.offset = dragPoint - (this.state.grabPoint || 0);
    this.syncTimelinePosition();
  ...
  render() {
    const { dragging, potentialTime, internalValue } = this.state;
    const {
      tickSpacing,
      allowedRange,
      disabled,
