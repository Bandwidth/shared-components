...
interface DragBoxCallbacks {
   ...
   */
  onMouseUp: (collisions: Set<string>) => void;
  /**
...
interface DragBoxProps extends DragBoxCallbacks {
  /**
   ...
   */
  disablePointerEventsWhileDragging: boolean;
}
 ...
 */
class DragBox extends React.Component<DragBoxProps, DragBoxState> {
  state: DragBoxState = {
    mouseDown: false,
    start: null,
    end: null,
  ...
  getScroll = (): { scrollLeft: number; scrollTop: number } =>
    pick(this.getScrollElement(), ['scrollLeft', 'scrollTop']);

  attachScrollSelector = () => {
    const el = this.getScrollElement();
    el && el.addEventListener('scroll', this.throttledScroll);
  ...
  detachScrollSelector = () => {
    const el = this.getScrollElement();
    el && el.removeEventListener('scroll', this.throttledScroll);
  ...
  getMousePosition = (ev, customScroll: ScrollPosition = null): Position => {
  ...
  addScroll = ({ x, y }, customScroll: ScrollPosition = null): Position => ({
  ...
  calcRect = () => {
    const { start, end, scrollLeft, scrollTop } = this.state;
    if (!start || !end) return {};
    return {
  ...
  checkChildrenBoxCollisions = () => {
    ...
    const colliding: string[] = collidingNodes.map(node =>
      node.getAttribute('data-drag-box-key'),
    );
    const notColliding: string[] = notCollidingNodes.map(node =>
      node.getAttribute('data-drag-box-key'),
    );
  ...
  refDragElement: any = (el: HTMLDivElement) => (this.dragElement = el);
