...
const Font = ({ theme, name }) => (
  ...
  >
    {text.map(t => (
      <Section key={t}>
        <P spacing="lg">{Array.from(t).join(' ')}</P>
      </Section>
    ))}
  </div>
);
...
const BlueLine = props => (
  <div
    style={{
      pointerEvents: 'none',
      float: 'left',
      borderBottom: '1px dashed rgba(0, 0, 255, 0.25)',
      height: '1px',
      width: '100%',
      position: 'relative',
      ...props.style,
...
const Space = () => (
  <div>
    ...
    <Section>
      <H1 spacing="lg">Heading with Spacing Applied</H1>
    </Section>
    ...
    <Section>
      <P spacing="lg">
        A line with an <Link>inline link</Link>
      </P>
    </Section>
  </div>
);
